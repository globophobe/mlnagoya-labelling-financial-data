import { Layout, Image, Appear, Fit, Fill, Heading, Table, TableHeader, TableHeaderItem, TableBody, TableRow, TableItem } from 'spectacle';
...
# The *cusum_filter* reduces the price series to only data points exceeding the threshold.
## *cusum_filter* は、価格系列をしきい値を超えるデータポイントのみに減らします。
...
## These are 3 "triple-barrier" touches.
## 3つの「トリプルバリア」タッチです。
...
## ラベル-1は証拠金取引を意味します。 持っていないものを売る。

---

# For example, using ¥100,000 as margin (collateral) to sell ¥200,000.
## たとえば、100,000円をマージン（担保）として使用して、200,000円を販売します。

---

# Label 0 suggests not enough volatility for trading to be worthwhile.
## ラベル0は、取引に値するボラティリティが十分でない事を示しています。
